...
export function validateEnvironment(): ValidationResult {
  ...
  if (frontendUrl) {
    try {
      ...
      errors.push(
        `Invalid FRONTEND_URL format: ${frontendUrl}. Must be a valid URL`,
      );
    }
  }
  ...
  if (jwtSecret) {
    ...
    if (jwtSecret === 'your-super-secret-jwt-key-change-in-production') {
      warnings.push(
        'JWT_SECRET appears to be the default value. Change it in production!',
      );
    }
  }
  ...
  if (nodeEnv && !['development', 'production', 'test'].includes(nodeEnv)) {
    warnings.push(
      `NODE_ENV has unusual value: ${nodeEnv}. Expected: development, production, or test`,
    );
  }
  ...
  if (forceRuleBased) {
    ...
    warnings.push(
      'OPENAI_API_KEY not set. AI agent features will be disabled. Set FORCE_RULE_BASED=true to explicitly use rule-based mode for testing.',
    );
  }
  ...
  if (transcriptionProvider === 'whisper' && !openaiKey) {
    warnings.push(
      'TRANSCRIPTION_PROVIDER=whisper but OPENAI_API_KEY not set. Whisper transcription will be unavailable.',
    );
  }
  ...
  if (transcriptionProvider === 'aws-transcribe') {
    ...
    if (!awsKey || !awsSecret) {
      warnings.push(
        'TRANSCRIPTION_PROVIDER=aws-transcribe but AWS credentials not set. AWS Transcribe will be unavailable.',
      );
    }
    if (!awsRegion) {

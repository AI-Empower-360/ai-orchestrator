...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor(private authService: AuthService) {
    console.log(
      '[JWT Strategy] Constructor called - JWT Strategy is being instantiated',
    );
    super({
    ...
    console.log(
      '[JWT Strategy] Strategy configured with secret:',
      (
        process.env.JWT_SECRET || 'your-secret-key-change-in-production'
      ).substring(0, 10) + '...',
    );
  ...
  async validate(payload: any): Promise<Doctor> {
    ...
    debugLog(
      ...
      { hasPayload: !!payload, payloadSub: payload?.sub },
      'H',
    );
    // #endregion
    const doctor = await this.authService.validateToken(payload);
    console.log(
      '[JWT Strategy] validate result:',

...
export class ClinicalAlertAgentService {
  ...
  async analyzeAndGenerateAlerts(
    ...
    if (!this.llmService.isAvailable()) {
      this.logger.log(
        'Using rule-based clinical alert generation (LLM not available)',
      );
...
Return as JSON array of alerts. If no alerts are needed, return empty array.`,
      ...
      this.logger.log(
        `Generated ${response.alerts?.length || 0} clinical alerts via AI`,
      );
      return response.alerts || [];
    } catch (error: any) {
      this.logger.error(
        `Failed to generate clinical alerts via AI: ${error.message}`,
        error.stack,
      );
  ...
  private getRuleBasedAlerts(
    ...
    const criticalKeywords = [
      {
        pattern: 'chest pain',
        message: 'Chest pain reported. Rule out cardiac event.',
      },
      {
        pattern: 'difficulty breathing',
        message:
          'Difficulty breathing reported. Assess respiratory status immediately.',
      },
      {
        pattern: 'shortness of breath',
        message: 'Shortness of breath reported. Monitor oxygen saturation.',
      },
      {
        pattern: 'severe pain',
        message:
          'Severe pain reported. Assess pain level and consider immediate intervention.',
      },
      {
        pattern: 'unconscious',
        message:
          'Unconscious state reported. Immediate medical attention required.',
      },
      {
        pattern: 'cardiac arrest',
        message:
          'Cardiac arrest mentioned. Activate emergency response immediately.',
      },
      {
        pattern: 'stroke',
        message:
          'Stroke symptoms mentioned. Time-sensitive evaluation required.',
      },
      {
        pattern: 'seizure',
        message:
          'Seizure activity reported. Monitor and assess neurological status.',
      },
      {
        pattern: 'anaphylaxis',
        message:
          'Anaphylaxis suspected. Immediate epinephrine administration may be required.',
      },
    ];
    ...
    if (alerts.length === 0) {
      const warningKeywords = [
        {
          pattern: 'high blood pressure',
          message:
            'Elevated blood pressure noted. Monitor and consider treatment.',
        },
        {
          pattern: 'hypertension',
          message: 'Hypertension mentioned. Review blood pressure readings.',
        },
        {
          pattern: 'fever',
          message:
        ...
        },
        {
          pattern: 'abnormal',
          message:
            'Abnormal finding mentioned. Further evaluation recommended.',
        },
        {
          pattern: 'elevated',
          message: 'Elevated value mentioned. Review and monitor.',
        },
        {
          pattern: 'medication interaction',
          message:
            'Potential medication interaction identified. Review medication list.',
        },
        {
          pattern: 'allergy',
          message: 'Allergy mentioned. Verify and document in patient record.',
        },
        {
          pattern: 'diabetes',
          message:
            'Diabetes mentioned. Monitor glucose levels and review management.',
        },
      ];
